...
constexpr int ATTN_N = 64; // sequence length
...
__global__ void attend_ker(const attn_globals<D> g) {
    ...
    load_global_to_shared_direct<1, false, st_bf<N_STEP, ATTN_D>, _gl_QKVO, coord<st_bf<N_STEP,ATTN_D>>, NUM_THREADS>(
        g.Kg, {batch_idx, 0, head_idx, 0}, k_smem);
    load_global_to_shared_direct<1, false, st_bf<N_STEP, ATTN_D>, _gl_QKVO, coord<st_bf<N_STEP,ATTN_D>>, NUM_THREADS>(
        g.Vg, {batch_idx, 0, head_idx, 0}, v_smem);
    ...
    load_lds_reg(k_reg, subtile_inplace<BLOCK_SIZE, ATTN_D>(k_smem, {0, 0}));
    load_lds_reg_col(v_reg, subtile_inplace<BLOCK_SIZE, ATTN_D>(v_smem, {0, 0}));
    // load<1, qkvo_tile<D, bf16>, _gl_QKVO>(k_reg, g.Kg, {batch_idx, tile_idx, head_idx, 0});
    // load<1, qkvo_tile<D, bf16, col_l>, _gl_QKVO>(v_reg, g.Vg, {batch_idx, tile_idx, head_idx, 0});
    ...
    store<1, qkvo_tile<D, float, accum_l>, _gl_QKVO, coord<qkvo_tile<D, float, accum_l>>>(g.Og, o_reg, {batch_idx, tile_idx, head_idx, 0});
}
