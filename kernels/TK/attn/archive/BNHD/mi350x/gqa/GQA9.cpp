...
__global__ void attend_ker(const attn_globals<D> g) {
    ...
    typename attn_tile<D, float, accum_l>::row_vec max_vec[2], norm_vec;
    ...
    col_max(max_vec[0], att_block[0]);
    sub_col(att_block[0], att_block[0], max_vec[0]);
    ...
    col_max(max_vec[1], att_block[1], max_vec[0]);
    sub_col(att_block[1], att_block[1], max_vec[1]);
    ...
    for (int j = 3; j < num_tiles - 1; j += 2) {
        ...
        sub(max_vec[0], max_vec[0], max_vec[1]); 
        exp2(max_vec[0], max_vec[0]);  
        mul(norm_vec, norm_vec, max_vec[0]);
        ...
        asm volatile("s_waitcnt lgkmcnt(0)");
        __builtin_amdgcn_s_setprio(1);
        mul_col(o_reg, o_reg, max_vec[0]);
        mma_AtB(o_reg, v_reg, att_block_col_bf16, o_reg);
        //      Partial softmax for QK2
        // copy(max_vec_prev, max_vec);
        col_max(max_vec[0], att_block[0], max_vec[1]);
        sub_col(att_block[0], att_block[0], max_vec[0]);
        exp2(att_block[0], att_block[0]);
        __builtin_amdgcn_s_setprio(0);
        __builtin_amdgcn_sched_barrier(0);
        ...
        mma_AtB(att_block[1], k_reg_transposed, q_reg_transposed, att_block[1]);
        //      Finish softmax for QK0
        sub(max_vec[1], max_vec[1], max_vec[0]); 
        exp2(max_vec[1], max_vec[1]);  
        mul(norm_vec, norm_vec, max_vec[1]);
        col_sum(norm_vec, att_block[0], norm_vec);
        copy(att_block_bf16, att_block[0]);
        att_block_col_bf16 = *(attn_tile<D, bf16, col_l>*)(&att_block_bf16);
        __builtin_amdgcn_sched_barrier(0);
